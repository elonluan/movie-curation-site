...
</svg>`;return`data:image/svg+xml;charset=UTF-8,${encodeURIComponent(n)}`}function m(t,{className:r="movie-card",withIntro:a=!1,scoreMode:s="total"}={}){const e=b(t,s),n=x(s),c=$(t.watchDate),d=`${n} ${e.toFixed(1)} · 观影日期 ${c}`,o=String(t.logline||"").trim(),i=document.createElement("a");return i.href=`/movie.html?id=${encodeURIComponent(t.id)}`,i.className=r,i.innerHTML=`
    <div class="poster-wrap">
      <img src="${g(t)}" alt="${t.titleZh} 海报" loading="lazy" />
    </div>
    <div class="card-body">
      <h3 class="card-title">${t.titleZh}</h3>
      ${o?`<p class="card-logline">${o}</p>`:""}
      <p class="card-subtitle">${t.titleOriginal}</p>
      <p class="card-year">${t.year}</p>
      <p class="card-meta">${d}</p>
      <div class="tag-row">
        ${t.tags.slice(0,3).map(u=>`<span class="tag">${u}</span>`).join("")}
      </div>
      ${a&&!o?`<p class="card-subtitle" style="margin-top: 12px;">${t.logline}</p>`:""}
    </div>
  `,i}function y(t){var r;return typeof((r=t==null?void 0:t.summaryScores)==null?void 0:r.total)=="number"&&Number.isFinite(t.summaryScores.total)?t.summaryScores.total:typeof(t==null?void 0:t.rating)=="number"&&Number.isFinite(t.rating)?t.rating*10:0}function b(t,r){const a=(t==null?void 0:t.summaryScores)??{};return r==="personal"?l(a.personal):r==="art"?l(a.art):r==="external"?l(a.external):y(t)}function x(t){return t==="personal"?"个人维度":t==="art"?"艺术维度":t==="external"?"外部维度":"最终得分"}function l(t){return typeof t=="number"&&Number.isFinite(t)?t:0}function $(t){if(!t)return"未记录";const r=new Date(t);if(Number.isNaN(r.getTime()))return t;const a=r.getFullYear(),s=String(r.getMonth()+1).padStart(2,"0"),e=String(r.getDate()).padStart(2,"0");return`${a}-${s}-${e}`}function S(){const t=document.querySelector("[data-year]");t&&(t.textContent=new Date().getFullYear())}export{m as c,w as l,S as m,g as p};
